...
\label{sec:td}
NOTE: this section is very much not finished, just added some very quick things on the notation used
in this paper.

% TODO: Merging
Given the complete graph $G'=(V,E)$. All tours $E'_j \subset E$, for $1 \leq j \leq} \#tours$ as found by the
heuristics are merged together into a new graph $G = (V, (\bigcup E'_j)}$).

% TODO: what is a tree decomposition
Todo: What is a tree decomposition (a tree $T=(W, F)$ with three properties \dots)

Let $X_i \subset V, i \in W$ be the set of vertices assigned to the $i$-th bag, and $Y_i =
\{(u,v) \in E: u,v \in X_i \}$ the set of edges corresponding to the $i$-th bag. A bag $i$ is
said to contain a vertex $v$ or edge $e$ if $v \in X_i$ or $e \in Y_i$ respectively.
...
explain their details.
    ...
    decomposition with width $k - 1$ and $X_i$ and $Y_i$ as defined in section~\ref{sec:td}.
    ...
    The algorithm then consists of a top down approach where we tabulate all entries for $F$,
    starting at the root and then recursively work downwards in the tree. Then the value of each
    table entry is finished bottem up as the recursion returns the values for children.
