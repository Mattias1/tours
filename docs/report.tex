...
% SWEEP????????
    ...
    even if the next edges would have weight 0 (in which case we failed). An example is shown in
    Figure~\ref{fig:lk-example}.
    ...
    The candidate set is further modified to always include edges that are in both of the two
    previous best solutions and try them first.

    The candidate for the edge $x_1$ is also changed so that no edges from the previous best tour
    are removed in the first level of the improvement step.
