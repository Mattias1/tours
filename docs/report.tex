...
\begin{document}
\tikzstyle{vertex2} = [circle,fill=black!25,minimum size=18pt,align=center,font=\tiny,inner sep=0pt]
...
\tikzstyle{bag} = [circle,fill=black!25,minimum size=35pt,align=center,text width=35pt,font=\tiny,inner sep=0pt]
...
to merge the found approximate solutions and calculate the optimal solution on the merged graph,
using branch decompositions. % TODO
...
are few that can compete with (variants of) the Lin-Kernighan heuristic~\cite{lin-kernighan, NO,
CITE SOMETHING ABOUT THE COMPETING PART HERE}, most notably the implementation of
Helsgaun~\cite{lkh2}. In this thesis we use the source code of Helsgaun's
implementation~\cite{lkh-url}, which is available for academic use.
...
For the TSP we chose to use the Lin-Kernighan-Helsgaun heuristic because it is one of the best
heuristics available and is available for academic use in both binary and source code. We will
discuss the Lin-Kernighan heuristic and the Lin-Kernighan-Helsgaun variant in Section~\ref{sec:lkh}
and Section~\ref{sec:lk}.
...
% SWEEP????????
    ...
    Another difference with the fixed $k$-opt algorithms is that in the 2-opt algorithm we first
    choose the set $X$ of edges in the original tour (i.e. two crossing edges with the euclidean
    metric) and then find the corresponding set $Y$ of new edges to be inserted in the tour. In the
    Lin-Kernighan algorithm, the sets $X$ and $Y$ are built up step by step, as the sequence grows.
    In the algorithm we start by choosing an initial vertex and choose one of the two adjacent edges
    $x_1$ in the original tour. After that we choose an edge $y_1$ from a set of edges to the 5
    nearest neighbours and the accompanying edge $x_1$ (which is uniquely defined in most cases, see
    Figure~\ref{fig:unique-x}).
    Note that the result after removing the edges in $X$ and adding the edges in $Y$ is not a tour,
    but a path. It can be closed however by adding the edge $y_2$ between the vertex after $x_2$ and
    the vertex before $x_1$.
    We keep adding edges $y_i$ and removing edges $x_{i+1}$, untill either the complete sequence
    (including the edge $y_{i+1}$ that will close the tour if added) is shorter than the previous
    tour (in which case we succeeded) or that there is no edge $y_i$ to find that improves the tour
    even if the next edges would have weight 0 (in which case we failed).
    If we fail, we can either stop, start again from scratch, or use backtracking. The latter means
    that we go back a few steps and try another edge $y_i$ (or occasionally another edge $x_{i+1}$).
    An example of this procedure is shown in Figure~\ref{fig:lk-example}.

    After we are done, we also try to apply the double-bridge move (see
    Figure~\ref{fig:double-bridge}) to improve the tour a bit more. This is the most basic move that
    cannot be found by Lin-Kernighans normal procedure (because intermediate steps will result in a
    broken tour) but does produce a valid tour as a whole. Lin and Kernighan found that in some
    cases this improved the result significantly, while in other cases it did nothing. It doesn't
    hurt to try though, as it is a relatively cheap optimization.
    % TODO: is non-sequential defined??? (by me)

    This concludes our overview of the Lin-Kernighan heuristic. We remark that this is only a short
    description of the algorithm and omits many details which are essential for the efficiency
    of the algorithm, as it is not a simple algorithm to implement~\cite{lkh1}. For a detailed
    description we refer to the original paper~\cite{lin-kernighan}.
    % TODO: 4 opt bridge move?
    % Quote from LKH 1:
    %   However, the Lin-Kernighan algorithm is not simple to implement. In a survey paper from
    %   1989 [14] the authors wrote that no other implementation of the algorithm at that time
    %   had shown as good efficiency as was obtained by Lin and Kernighan.
    ...
    \begin{tabular}{c c}
        ...
        \node[vertex0] (3) at (4.0, -1.0) {};
        \node[vertex0] (4) at (4.0, -2.0) {};
        ...
        \path[edge,bend left=33] (2) to (3);
        \path[edge,bend left=12] (3) to (4);
        ...
        \node[vertex1] (1) at (1.4, 0.1) {};
        \node[vertex1] (2) at (2.0, 0.3) {};
        \node[vertex1] (3) at (4.1, -1.2) {};
        \node[vertex1] (4) at (4.1, -1.8) {};
        \node[vertex1] (5) at (1.7, -3.2) {};
        \path[edge,bend left=48] (0) to (1);
        \path[edge] (0) to node[below] {$x_1$} (5);
        \path[edge] (0) to node[right] {$y_1$} (2);
    ...
    \centering
    \begin{tikzpicture}[auto,swap]
    \node[vertex1] (0) at (6.9, -0.79) {};
    \node[vertex1] (1) at (7.8, -0.77) {};
    \node[vertex1] (2) at (6.9, -4.8) {};
    \node[vertex1] (3) at (7.8, -4.8) {};
    \node[vertex1] (4) at (9.4, -2.4) {};
    \node[vertex1] (5) at (9.4, -3.2) {};
    \node[vertex1] (6) at (5.3, -2.4) {};
    \node[vertex1] (7) at (5.3, -3.2) {};
    ...
    1-tree containing $e$ and the length of the minimum 1-tree. Or in other words: the increase in
    length of the minimum 1-tree if it is required to contain the edge $e$.
    ...
    every move was composed as a sequence of 2-opt moves (and possibly a 3-opt). Helsgaun modified
    it to use moves that consist of 5-opt moves (unless a $k$-opt move for smaller $k$ results in an
    improvement already). Later he modified it again to use general $k$-opt moves up to a certain
    $k$ that you can choose yourself as the basic step. The steps to do this are rather involved and
    contain a large case-analysis and won't be described here. For the details we refer to the paper
    of Helsgaun~\cite{lkh2}. The main idea of his approach however is to allow edges $x_i$ that
    initially break up the tour (like in Figure~\ref{fig:unique-x}) and restrict the later edges
    $y_{i+1}$ and $x_{i+1}$ so that the tour can be fixed again. This allows, amoung others, the
    double bridge move from Figure~\ref{fig:double-bridge} to be included natively in the search.
    ...
    different initial tours. These tours where constructed randomly because construction heuristics
    where considered unnescessary. Helsgaun notes that even though a good construction heuristic
    doesn't improve the quality of the final tour a lot, it does improve the running time of the
    algorithm.
    The heuristic used by Helsgaun tries to construct a tour greedily by choosing edges that either
    have an $\alpha$-nearness value of 0 and that are in the previous best tour. If no such edge is
    found either a random other candidate edge is chosen or, if they are not valid either, an edge
    to any free vertex.

    % Main points from LKH 2 paper:
    % Partitioning
...
the defenition of a tree decomposition.
    ...
    Calculating the optimal treewidth or the optimal tree decomposition is an NP Hard problem
    \cite{find ref}, so finding an optimal decomposition in reasonable time is infeasable unlesss
    $P=NP$. We do not nescessarily need a tree decomposition of optimal width, we just need the
    ...
    which is a simple but effective heuristic. It quickly obtains results close to the optimum and
    ...
    tours, and therefore every vertex is guaranteed to have at least two neighbours. This is
    specific to TSP and VRP.
...
\label{sec:dp}
Provided the width is small enough, the optimal solution for TSP or VRP on the merged graph can be
computed using a dynamic programming algorithm on the tree decomposition of the graph. In the
