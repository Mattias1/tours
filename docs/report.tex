...
Helsgaun~\cite{lkh2}.
To find good initial solutions for the TSP we chose to use the Lin-Kernighan-Helsgaun heuristic
...
% TODO: With possibly an optimization using LKH - and possibly multiple pertubated runs of savings.
    ...
    \begin{enumerate}
        \item Generate a (possibly random) initial tour.
    ...
    For $x_{i+1}$ we choose the edge with the largest weight. Other choices are initially ignored
    ...
    optimizations below, one to speed up the algorithm, the other to improve the solution. We remark
    that there are many more details which are essential for the efficiency of the algorithm, as it
    is not a simple algorithm to implement~\cite{lkh1}. We omit them here as they are not essential
    to this thesis. For the exact details we refer to the original paper~\cite{lin-kernighan}.

    The first optimization reduces the choices for the edges. In the first few solutions found by
    the algorithm there always are a lot of edges that are in every solution. These common edges are
    recorded and then after the first solutions they are no longer allowed to be broken. This means
    they cannot be used as choice for $x_{i+1}$ any more, which in turn limits the choices for $y_i$
    and speeds up the overall algorithm by a factor of 5. Note that in order to not bias the
