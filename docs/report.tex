...
% SWEEP????????
    ...
    The first major thing that is changed is the candidate set for the edges in $Y$. In the original
    algorithm the choices for an edge $y_i$ are limited to the edges to the first 5 nearest
    neighbours. The choice for this candidate set assumes that the shorter an edge is, the higher
    the chance is that it occurs in a tour. This is reasonable, but it is not always a good
    estimation. Helsgaun notices that the distance to a minimum 1-tree, called
    ...
    previous best solutions and try them first.
    The candidate set for the first edge, $x_1$, is also changed so that no edges from the previous
    best tour are removed in the first level of the improvement step.
    ...
    every move was composed as a sequence of 2-opt moves. Helsgaun modified it to use moves that
    consist of 5-opt moves (unless a $k$-opt move for smaller $k$ results in an improvement
    already). Later he modified it again to use general $k$-opt moves up to a certain $k$ as the
    basic step. The steps to do this are rather involved and contain a large case-analysis and won't
    be described here. For the details we refer to the paper of Helsgaun~\cite{lkh2}. The main idea
    of his approach however is to allow edges $x_i$ that initially break up the tour (like in
    Figure~\ref{fig:unique-x}) and restrict the later edges $y_{i+1}$ and $x_{i+1}$ so that the tour
    can be fixed again. This allows, amoung others, the double bridge move from
    Figure~\ref{fig:double-bridge} to be included natively in the search.
    ...
    % Main points from LKH 1 paper:
    TODO:
    - Initial tours...

    % Main points from LKH 2 paper:
    - Partitioning                        (Ignored)
    - Tour merging                        (Ignored)
    - Iterative partial transcription     (Ignored)
    - Backbone-guided search              (Ignored)
