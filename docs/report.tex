...
% TODO: roadmap
    ...
    vertices in $X_i$ and $M_i$ a matching. If there is an edge $\{u, v\} \in M_i$ then there should
    be a path that starts in $u$ and ends in $v$. All vertices in $X_i$ itself should have degrees
    as given in the degrees parameter, and the vertices that occur only in the bags below $X_i$ in
    the tree should all be used (have a degree of 2).
    ...
    % $X_i$ (in the tree) if $i$ is on the path from $j$ to the root of $T$.
    The defenition of the instruction-function $F(X_i, D_i, M_i)$ remains the same except that the
